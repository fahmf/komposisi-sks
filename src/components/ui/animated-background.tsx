...
const WAVES: {
...
}[] = [
  {
    ...
    gradient: 'radial-gradient(ellipse 800px 600px at 50% 20%, rgba(58, 172, 131, 0.45) 0%, transparent 50%)',
    animation: 'aurora1 26s ease-in-out infinite alternate',
    ambient: 'opacity-[0.45] dark:opacity-[0.40]',
    vivid: 'opacity-70',
  },
  {
    // Violet — mirrors the ILL badges used across the app.
    gradient: 'radial-gradient(ellipse 600px 400px at 80% 30%, rgba(139, 92, 246, 0.40) 0%, transparent 50%)',
    animation: 'aurora2 32s ease-in-out infinite alternate-reverse',
    ambient: 'opacity-[0.28] dark:opacity-[0.32]',
    vivid: 'opacity-50',
  },
  {
    // Teal — cools the middle of the field.
    gradient: 'radial-gradient(ellipse 700px 500px at 20% 60%, rgba(45, 212, 191, 0.38) 0%, transparent 50%)',
    animation: 'aurora3 38s ease-in-out infinite alternate',
    ambient: 'opacity-[0.30] dark:opacity-[0.28]',
    vivid: 'opacity-40',
  },
  {
    // Deep pine — grounds the lower edge.
    gradient: 'radial-gradient(ellipse 900px 300px at 60% 80%, rgba(31, 143, 104, 0.35) 0%, transparent 50%)',
    animation: 'aurora4 29s ease-in-out infinite alternate-reverse',
    ambient: 'opacity-[0.26] dark:opacity-[0.30]',
    vivid: 'opacity-30',
  },
];
...
function AuroraLayers({ variant }: { variant: AuroraVariant }) {
  return (
    <>
      ...
      <div
        className={
          variant === 'vivid'
            ...
            : 'absolute inset-0 opacity-70 dark:opacity-60'
        }
      >
        {variant === 'ambient' && (
          <div className="absolute inset-0 bg-gradient-to-br from-brand-100/50 via-transparent to-violet-100/40 dark:from-brand-900/25 dark:via-transparent dark:to-violet-900/20" />
        )}
      </div>
      ...
      {/* Depth overlay so the top and bottom edges settle down. */}
      <div className="absolute inset-0 bg-gradient-to-t from-white/30 via-transparent to-white/10 dark:from-black/40 dark:via-transparent dark:to-black/20" />
    </>
  );
}
