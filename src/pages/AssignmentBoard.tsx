...
export default function AssignmentBoard() {
  ...
  const derived = useMemo(() => {
    ...
    return { sm, report, loads, assignmentBySlot, teacherTone, slotHardError, assignedCount };
  }, [plan, teachers, subjects]);

  if (!plan) return <EmptyState title="Belum ada semester aktif" hint="Buat semester di menu Pengaturan." />;
  if (!derived) return null;

  const { report, loads, assignmentBySlot, teacherTone, slotHardError, assignedCount } = derived;
  const groups = groupedClasses(plan);
  const totalSlots = plan.slots.length;

  const getCell = (classGroupId: string, slot: Slot) => {
    ...
    return { teacherId, locked, eligible, tone, currentName, classGroupId };
  };
  ...
  return (
    <>
      <PageHeader
        ...
        actions={
          <>
            <button className="btn-outline" onClick={() => { if (confirm('Kosongkan semua penugasan?')) clearAssignments(); }}>
      ...
      {totalSlots === 0 ? (
        <EmptyState
          ...
          action={<Link to="/classes" className="btn-primary">Ke menu Kelas</Link>}
        />
      ) : (
        <>
          ...
          <div className="mb-6 grid gap-2 sm:grid-cols-2 lg:grid-cols-3">
            {sortedTeachers.map((t) => {
              ...
              const barColor = tone === 'error' ? 'bg-rose-500' : tl.load >= tl.relaxedTarget ? 'bg-emerald-500' : tone === 'warn' ? 'bg-amber-400' : 'bg-brand-500';
              return (
                <Link key={t.id} to={`/teacher/${t.id}`} className="card p-3 transition hover:shadow-md">
                  ...
                  <p className="mt-1 text-[11px] text-slate-400">
                    target {tl.relaxedTarget} SKS{tl.load >= tl.relaxedTarget ? ' ✓' : ''}
                  </p>
                </Link>
              );
            })}
                    ...
                    <tbody>
                      {g.classes.map((c) => (
                        <tr key={c.id} className="border-t border-slate-100 dark:border-slate-800">
                          ...
                          {columns.map((e) => {
                            const slot = plan.slots.find((s) => s.id === makeSlotId(c.id, e.subjectId));
                            if (!slot) return <td key={e.id} />;
                            const cell = getCell(c.id, slot);
                            return (
